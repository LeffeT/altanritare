...
const APP_VERSION = "v1.53 · trekantslist i Virke & cc-avstånd";
...
function computeBom(p) {
  ...
  const coverStart = Math.min(Math.max(0, p.roof.coverStart || 0), Math.max(0, edgeRun - 0.3));
  const coveredRun = Math.max(0.1, edgeRun - coverStart);
  const coveredSlope = coveredRun / Math.cos(d.angleRad);          // täckt del (råspont/papp/läkt/plåt)
  const battenRows = Math.max(2, Math.ceil((coveredSlope * 1000) / Math.max(100, f.battenCC)) + 1);
  ...
  const roofArea = roofW * coveredSlope;
  const covering = f.roofType || "TP20 plåt";
  const isKlick = /klick/i.test(covering);
  const plateWidth = f.plateWidth || 1.0; // täckande (nyttig) bredd per plåt, efter sidoöverlapp
  const seams = Math.max(0, Math.ceil(roofW / plateWidth) - 1);
  const platesAcross = seams + 1;
  const plateLen = f.plateLen || 3.5;
  const platesAlong = Math.max(1, Math.ceil(coveredSlope / plateLen));
  const platesTotal = platesAcross * platesAlong;
  const purchasedPlateArea = platesTotal * plateLen * plateWidth; // faktisk köpt yta (hela plåtar, inkl. kaputtag)
  const pappRollArea = f.pappRollArea || 20; // m² per rulle (kontrollera mot vald produkt)
  const pappRolls = Math.max(1, Math.ceil(roofArea / pappRollArea));
  const pappPurchasedArea = pappRolls * pappRollArea; // faktisk köpt yta (hela rullar)
  const trekantLen = f.trekantLen || 3.6; // lagerlängd trekantslist
  const trekantPiecesPerRow = Math.max(1, Math.ceil(roofW / trekantLen));
  const trekantBoards = trekantPiecesPerRow; // en rad längs takfoten
  const farmar = Math.ceil(roofArea * (isKlick ? 6 : 9));
...
const PRICE_DEFAULTS = {
  ...
  "Ströläkt": 9, "Bärläkt": 16, "Trekantslist": 25, "Farmarskruv": 1.2, "Överlappsskruv": 0.8, "Butylband": 14,
};
// Pris (kr/m²) per taktäckningstyp – förval, baseras på faktiskt köpt (hela) plåtyta
const COVERING_PRICE_BY_TYPE = { "TP20 plåt": 190, "Klicktak": 220 };
...
// Räknar antal brädor/skivor råspont utifrån dimension och takyta
function raspontBoardCount(dim, areaM2) {
  const d = String(dim || "");
  const spillPct = 10; // vanlig riktlinje för spill/kapspill vid inköp
  const mLucka = d.match(/(\d+)\s*×\s*(\d+)/); // t.ex. "540×3600"
  if (/LUCKA/i.test(d) && mLucka) {
    const widthMm = parseInt(mLucka[1], 10);
    const lenM = parseInt(mLucka[2], 10) / 1000;
    const areaPerSheet = (widthMm / 1000) * lenM;
    const count = Math.ceil(areaM2 / areaPerSheet);
    return { count, withSpill: Math.ceil((areaM2 * (1 + spillPct / 100)) / areaPerSheet), lenM, widthMm, spillPct };
  }
  const p = d.split("×");
  const widthMm = parseFloat(p[1]) || 95;
  const lenM = 3.6; // vanlig lagerlängd för spontad råspont
  const areaPerBoard = (widthMm / 1000) * lenM;
  const count = Math.ceil(areaM2 / areaPerBoard);
  return { count, withSpill: Math.ceil((areaM2 * (1 + spillPct / 100)) / areaPerBoard), lenM, widthMm, spillPct };
}
...
function bomRows(p) {
  ...
  const f = p.frame;
  const rows = [
    { name: b.covering, dim: `${num(b.plateLen)} × ${num(b.plateWidth)} m per skiva`, antal: `${num(b.purchasedPlateArea)} m²`, detalj: `${b.platesTotal} st (${b.platesAcross} brett${b.platesAlong > 1 ? ` × ${b.platesAlong} i längd, skarvas` : ""}) · taklutning ca ${num(b.angleDeg)}°${b.coverStart > 0.01 ? ` · börjar ${num(b.coverStart)} m från huset` : ""} · takyta som täcks: ${num(b.roofArea)} m²`, pq: b.purchasedPlateArea, pu: "m²", priceFallback: COVERING_PRICE_BY_TYPE[b.covering] },
    ...
    { name: "Takpapp", dim: f.papptyp || "YEP 2500", antal: `${b.pappRolls} rullar`, detalj: `underlagspapp på råspont · à ${num(b.pappRollArea)} m²/rulle · täcker ${num(b.pappPurchasedArea)} m² (behov ${num(b.roofArea)} m²)`, pq: b.pappPurchasedArea, pu: "m²" },
    { name: "Ströläkt", dim: f.stroDim || "25×48", antal: `ca ${num(b.stroPurchasedLen)} lpm`, detalj: `cc ${f.stroCC || 600} mm · längs takfallet · behov ca ${num(b.stroLM)} lpm · ${b.stroBoards} st à ${num(b.stroLektLen)} m${b.stroPiecesPerStrip > 1 ? ` (${b.stroPiecesPerStrip} per läkt, skarvas)` : ""}`, pq: b.stroPurchasedLen, pu: "lpm" },
    { name: "Bärläkt", dim: f.battenDim, antal: `ca ${num(b.battenPurchasedLen)} lpm`, detalj: `cc ${f.battenCC} mm · ${b.battenRows} rader (takbredd ${num(b.roofW)} m inkl. sidoutskjut) · behov ca ${num(b.battenLM)} lpm · ${b.battenBoards} st à ${num(b.battenLektLen)} m${b.battenPiecesPerRow > 1 ? ` (${b.battenPiecesPerRow} per rad, skarvas)` : ""}`, pq: b.battenPurchasedLen, pu: "lpm" },
    { name: "Trekantslist", dim: f.trekantDim || "45×45", antal: `${b.trekantBoards} st`, detalj: `vid takfoten · längd ${num(b.roofW)} m (takbredd inkl. sidoutskjut) · à ${num(b.trekantLen)} m${b.trekantPiecesPerRow > 1 ? ` (skarvas)` : ""}`, pq: b.trekantBoards * b.trekantLen, pu: "lpm" },
...
const DEFAULT_PROJECT = {
  ...
  roof: { width: 8, depth: 4, heightAtWall: 2.6, slope: 15, overhangFront: 0.3, overhangSide: 0.3, coverStart: 0, backBeam: "hus" },
...
function Sidebar({ project, set, openSection, setOpenSection, width = 320, onClose, onSave, onOpenProjects }) {
  ...
  const addOpening = (kind) => set((p) => {
    const o = newOpening(kind);
    if (kind === "door") o.sill = clamp(p.deck.height || 0, 0, Math.max(0, p.house.height - 0.3));
    return { ...p, house: { ...p.house, openings: [...(p.house.openings || []), o] } };
  });
              ...
              {openings.map((o, i) => {
                ...
                return (
                  <div key={o.id} style={{ background: T.panel2, border: `1px solid ${T.line}`, borderRadius: 11, padding: "11px 12px" }}>
                    ...
                    <NumberField label={isDoor ? "Underkant över mark (tröskel)" : "Underkant över mark"} value={o.sill} onChange={(v) => updateOpening(o.id, "sill", v)} min={0} max={Math.max(0, project.house.height - 0.3)} />
  ...
  useEffect(() => {
    ...
    const rebuild = (p) => {
      ...
      if (deck.hasRoof) {
        ...
        const cs = Math.min(Math.max(0, roof.coverStart || 0), Math.max(0, R - 0.3)); // takytan börjar här
        const covMidZ = (cs + R) / 2;
        const os = roof.overhangSide || 0;
        const roofW = roof.width + 2 * os;
        const slopeLen = Math.sqrt(R * R + dv.edgeDrop * dv.edgeDrop);
        const covSlope = slopeLen * ((R - cs) / R);
        const planeMidY = deckTop + roof.heightAtWall - dv.edgeDrop / 2;
        const planeYat = (z) => deckTop + roof.heightAtWall - (roof.slope / 100) * z;
        const planeCovMidY = planeYat(covMidZ);
        const rDepth = dv.rafterDepth || 0.22;

        // Takstolar – full längd, sitter kvar mot huset
        ...
        // Ströläkt – endast över den täckta delen (cs..R)
        ...
        for (let i = 0; i < nS; i++) {
          ...
          addBox(sWide, sThick, covSlope, px, planeCovMidY + sThick / 2 + 0.006, covMidZ, mat.batten, dv.angleRad);
        }

        // Bärläkt – endast täckta delen, hela takbredden inkl. sidoutskjut
        const bCC = Math.max(0.1, (frame.battenCC || 600) / 1000);
        const nB = Math.max(2, Math.floor(covSlope / bCC) + 1);
        for (let k = 0; k < nB; k++) {
          const z = cs + (k / (nB - 1)) * (R - cs);
...
function PriceInput({ value, unit, onChange }) {
...
function BomTable({ project, set }) {
  ...
  const roofArea = computeBom(project).roofArea;
  return (
    <div style={{ background: "#fff", border: "1px solid #d4d9df", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ padding: "11px 16px", borderBottom: "1px solid #e6e9ed", background: "#f7f8fa", display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
        <span style={{ fontSize: 13.5, fontWeight: 700, color: "#0f172a" }}>MATERIALLISTA MED KOSTNAD (UPPSKATTAD)</span>
        <span style={{ fontSize: 11.5, color: "#64748b", whiteSpace: "nowrap" }}>Takyta: {num(roofArea)} m²</span>
      </div>
      ...
      <div style={{ padding: "10px 16px", borderTop: "1px solid #e6e9ed", fontSize: 11, color: "#94a3b8" }}>
        À-priser är egna, redigerbara uppskattningar (kr per enhet) – skriv in dagspris från din bygghandel. Mängderna är den faktiska mängd som behöver köpas in (hela plåtar/läkt/brädor, inte bara teoretiskt behov) – ingen generell spillprocent behövs ovanpå. Kontrollera bärförmåga/snölast med konstruktör.
      </div>
    </div>
  );
}
...
const TREKANT_DIMS = ["45×45", "34×45", "50×50"];
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 22 }}>
      <div style={{ maxWidth: 760, margin: "0 auto", display: "grid", gap: 16 }}>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {heading("Trekantslist")}
                  <Select label="Dimension" value={f.trekantDim || "45×45"} onChange={(v) => setFrame("trekantDim", v)} options={TREKANT_DIMS} />
                  <Select
                    label="Lagerlängd"
                    value={`${num(f.trekantLen || 3.6)} m`}
                    onChange={(v) => setFrame("trekantLen", parseFloat(String(v).replace(",", ".")))}
                    options={LEKT_LENGTHS}
                  />
                  <div style={{ fontSize: 11.5, color: T.dim, margin: "-6px 0 4px" }}>
                    Läggs vid takfoten för att ge plåten rätt lutning ut mot droppkanten.
                  </div>
                  ...
                  <NumberField label="m² per rulle" unit="m²" value={f.pappRollArea || 20} onChange={(v) => setFrame("pappRollArea", v)} step={1} min={5} max={50} />
                  <div style={{ fontSize: 11.5, color: T.dim, margin: "-6px 0 4px" }}>
                    Står på rullens förpackning – vanligt är 20 m² (1 × 20 m) eller 10 m² (1 × 10 m).
                  </div>
        ...
        <BomTable project={project} set={set} />
      </div>
    </div>
  );
}
...
function RoofFramingDrawing({ project, width = 720 }) {
  ...
  const cs = Math.min(Math.max(0, roof.coverStart || 0), Math.max(0, R - 0.3));
  const covRun = R - cs;
  const M = 54, H = 440;
  ...
  const Y = (ym) => M + ym * s;
  const d = derive(project);
  const covSlope = covRun / Math.cos(d.angleRad || 0);
  const rCC = Math.max(0.1, (f.rafterCC || 600) / 1000);
  ...
  const battenYs = Array.from({ length: nB }, (_, i) => cs + (i / (nB - 1)) * covRun);
  ...
  return (
    <Paper title="LÄKTPLAN (takstomme uppifrån)" scaleNote={`${num(roofW)} × ${num(R)} m`} width={width} height={H}>
      ...
      {cs > 0.01 && <rect x={X(-roofW / 2)} y={Y(cs)} width={roofW * s} height={covRun * s} fill="#eef7f4" />}
      ...
      {colXs(nS).map((x, i) => (<line key={"s" + i} x1={X(x)} y1={Y(cs)} x2={X(x)} y2={Y(R)} stroke="#d68a2a" strokeWidth="2.2" />))}
...
function bomTableSvgBlock(rows, project, width) {
  ...
  b += `<text x="${pad}" y="${titleH - 11}" font-family="sans-serif" font-size="13" font-weight="700" fill="#0f172a">MATERIALLISTA MED KOSTNAD (UPPSKATTAD)</text>`;
  b += `<text x="${sumX}" y="${titleH - 11}" text-anchor="end" font-family="sans-serif" font-size="11" fill="#64748b">Takyta: ${num(roofArea)} m²</text>`;
  b += `<line x1="0" y1="${titleH}" x2="${width}" y2="${titleH}" stroke="#e6e9ed"/>`;
  const hy = titleH + headH - 7;
  b += `<text x="${cx.del}" y="${hy}" font-family="sans-serif" font-size="10.5" font-weight="600" fill="#64748b">DEL</text>`;
  b += `<text x="${cx.dim}" y="${hy}" font-family="sans-serif" font-size="10.5" font-weight="600" fill="#64748b">DIMENSION</text>`;
  b += `<text x="${cx.antal}" y="${hy}" font-family="sans-serif" font-size="10.5" font-weight="600" fill="#64748b">ANTAL</text>`;
  b += `<text x="${cx.apris}" y="${hy}" font-family="sans-serif" font-size="10.5" font-weight="600" fill="#64748b">À-PRIS</text>`;
  b += `<text x="${sumX}" y="${hy}" font-family="sans-serif" font-size="10.5" font-weight="600" fill="#64748b" text-anchor="end">SUMMA</text>`;
  ...
  rows.forEach((r, i) => {
    ...
    b += `<text x="${cx.dim}" y="${ty}" font-family="sans-serif" font-size="11.5" fill="#475569">${xmlEsc(r.dim)}</text>`;
    b += `<text x="${cx.antal}" y="${ty}" font-family="sans-serif" font-size="11.5" fill="#0f172a">${xmlEsc(r.antal)}</text>`;
    b += `<text x="${cx.apris}" y="${ty}" font-family="sans-serif" font-size="11" fill="#64748b">${xmlEsc(num(price))} kr/${xmlEsc(r.pu)}</text>`;
    b += `<text x="${sumX}" y="${ty}" font-family="sans-serif" font-size="11.5" font-weight="600" fill="#0f172a" text-anchor="end">${xmlEsc(kr(summa))}</text>`;
    if (i < rows.length - 1) b += `<line x1="0" y1="${y + rowH}" x2="${width}" y2="${y + rowH}" stroke="#eef1f4"/>`;
  });
  ...
  b += `<text x="${pad}" y="${totalBottom + 18}" font-family="sans-serif" font-size="9.5" fill="#94a3b8">Mängder är faktisk inköpsmängd (hela plåtar/läkt/brädor). À-priser är egna uppskattningar.</text>`;
  return { str: `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${h}" viewBox="0 0 ${width} ${h}">${b}</svg>`, w: width, h };
}
...
async function buildPdfUrl(project) {
  ...
  const bom = project.deck.hasRoof ? bomTableSvgBlock(bomRows(project), project, 760) : null;
